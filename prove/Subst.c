...
void _th_tree_print_unifier(struct _ex_unifier *u)
{
    ...
    for (i = 0; i < TABLE_SIZE; ++i) {
        e = u->table[i] ;
        while(e != NULL) {
            _tree_print("%s->%s ", _th_intern_decode(e->var), _th_print_exp(e->exp)) ;
            e = e->next ;
        }
    }
    printf("\n") ;
...
static struct _ex_intern *_subst(struct env *env,struct _ex_unifier *u, struct _ex_intern *e)
{
    ...
    switch (e->type) {
        ...
        case EXP_CASE:
            ...
            return _ex_intern_case(_subst(env,u,e->u.case_stmt.exp),e->u.case_stmt.count,args) ;
        case EXP_VAR:
